...
Flask 后端 backend_ros2.py(web_cmd_bridge 节点)
   ...
   │  发布 → /car01/cmd_vel_emergency (E-STOP,最高优先级直达 mux)
...
- **紧急停止**:单独大按钮,走 `cmd_vel_emergency`(优先级 255)。
...
若要上公网,应加:①登录/Token 鉴权;②HTTPS;③命令频率限制;④服务端急停兜底。
